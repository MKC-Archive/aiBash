...
setup_server() {
  ...
  MKC_txt "${blue}[INSTALL]:${green} php$phpVer-curl"
	...
	configFile='/etc/nginx/global/common.conf'

	echo "#Default">$configFile
	echo "server {">$configFile
	echo "listen   80 default;">$configFile
	echo "server_name $DOMAIN;">$configFile
	echo "location / {">$configFile
	echo "proxy_pass http://127.0.0.1:8080;">$configFile
	echo "proxy_set_header   X-Real-IP $remote_addr;">$configFile
	echo "proxy_set_header   Host $http_host;">$configFile
	echo "proxy_set_header   X-Forwarded-For $proxy_add_x_forwarded_for;">$configFile
	echo "}">$configFile
	echo "">$configFile
	echo "location ~*^.+\.(jpg|jpeg|gif|png|css|zip|tgz|gz|rar|bz2|doc|xls|exe|pdf|ppt|tar|wav|bmp|rtf|js)$ {">$configFile
	echo "root   $INSTALL_DIR;">$configFile
	echo "}">$configFile
	echo "}">$configFile
